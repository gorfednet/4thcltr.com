#!/usr/bin/env bash
# Deploy dist/ to an SMB share (e.g. /Volumes/data/websites/4thcltr.com).
# SSL is not configured here: it is handled on the server by nginx. Ensure the server
# has an nginx vhost with HTTPS and HTTP→HTTPS redirect; see nginx/4thcltr.com.conf.
...
if [ -n "$SMB_MOUNT" ]; then
  ...
  MNT="${SMB_MOUNT_POINT:-/Volumes/4thcltr-deploy}"
  ...
  echo "  SMB_MOUNT=/Volumes/data/websites/4thcltr.com   # already-mounted share"
  echo "  SMB_URL=//user@host/share                      # mount and deploy"
  exit 1
fi

rsync -av --delete-after "$DIST/" "$DEST/"
